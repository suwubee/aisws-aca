...
> - **AI CLI 模式**：用户在任务里明确选择 `cli_type`（claude/codex/gemini…），系统通过 **终端输出锚点** 判断“进入/退出 AI CLI”，并在 CLI 交互态下工作。
>
> 规则：**不会用“输入命令关键词”来判定进入 CLI**；必须以任务选择 + 输出锚点为准。
...
| S3 | 终端里你手动进入 CLI，再启用托管 | 任务绑定终端 + 输出锚点进入 CLI | 不新开终端，也能开始跟踪 | 任务 `cli_type` 默认值与实际不一致 |
...
- ✅ 进入 CLI：必须“任务选择 + 输出锚点命中”，不再用输入关键词
- ⚠️ 退出 CLI：当前用 `state -> idle` 启发式回落，可能误判（建议后续加强）
...
1. **终端面板“启用 AI 托管”未让用户选择 `cli_type`**  
   - 自动创建任务时默认 `claude`，用户实际用 `codex` 会永远 detected=false → 无法发送 prompt。
