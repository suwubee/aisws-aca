...
type TerminalSession struct {
	...
	ServerID        *string    `gorm:"index" json:"server_id"`
	ContinuedFromID *string    `gorm:"index" json:"continued_from_id,omitempty"` // 终端延续来源
...
// CLIExecution CLI 执行记录（用于 plan/execute/review 迁移期统一追踪）
type CLIExecution struct {
	ID                string     `gorm:"primaryKey" json:"id"`
	TaskID            *string    `gorm:"index" json:"task_id"`
	TerminalID        *string    `gorm:"index" json:"terminal_id"`
	WorkflowRunID     *string    `gorm:"index" json:"workflow_run_id"`
	WorkflowSessionID *string    `gorm:"index" json:"workflow_session_id"`
	ParentExecutionID *string    `gorm:"index" json:"parent_execution_id,omitempty"`
	Role              string     `gorm:"index" json:"role"`               // primary, review, replay, audit
	Tool              string     `gorm:"index" json:"tool"`               // claude, codex, gemini, shell
	Mode              string     `json:"mode"`                            // plan, execute, review, command
	Source            string     `gorm:"index" json:"source"`             // workflow, task, manual, api
	PromptPreview     string     `gorm:"type:text" json:"prompt_preview"` // 截断预览
	Status            string     `gorm:"index" json:"status"`             // running, completed, error, timeout, cancelled
...
// CLIExecutionEvent CLI 执行事件流水（发起/监控/记录）
type CLIExecutionEvent struct {
	Seq         uint64    `gorm:"primaryKey;autoIncrement" json:"seq"`
	ExecutionID string    `gorm:"index" json:"execution_id"`
	EventType   string    `gorm:"index" json:"event_type"`  // started, output, progress, completed, error, review
...
// AIWorkflowEvent AI 托管会话事件（开始/迭代/工具调用/状态变化）
type AIWorkflowEvent struct {
	ID         uint64    `gorm:"primaryKey;autoIncrement" json:"id"`
	SessionID  string    `gorm:"index" json:"session_id"`
	WorkflowID string    `gorm:"index" json:"workflow_id"`
	TaskID     *string   `gorm:"index" json:"task_id,omitempty"`
	TerminalID *string   `gorm:"index" json:"terminal_id,omitempty"`
	Iteration  int       `gorm:"index" json:"iteration"`
	Phase      string    `gorm:"index" json:"phase"`       // lifecycle, plan, execute, review, command
	EventType  string    `gorm:"index" json:"event_type"`  // session_started, step_started, tool_call, tool_result, paused, completed, failed...
