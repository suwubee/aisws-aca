...
### v1.1.2 - Terminal & 审批记录体验优化 ✅ 已完成 (2026-01-05)
- 终端日志去噪：处理 ANSI/回车覆盖/UTF-8 分片，避免输出叠字与乱码，保留中文显示
- 终端显示修复：浮层/全屏/侧栏切换时使用 `ResizeObserver` 自动 `fit()`，避免内容割裂
- 审批记录联动：终端侧边新增审批记录面板（按终端过滤）；设置页审批记录展示修复并增强格式化
- 审批内容清洗：落库前清理 ANSI 控制码/控制字符，优先保存提取后的审批提示片段，响应/规则/决策字段更稳定
